...
const feedbackErrorCloseBtn = document.getElementById(
  "feedback-error-close-btn"
);
...
const APPRECIATION_MESSAGES = [
  {
    title: "Wonderful! ✨",
    message: "One more moment of gratitude in your collection!",
  },
  {
    title: "Beautiful! 🌟",
    message: "Your jar is filling with positive energy!",
  },
  {
    title: "Fantastic! 💫",
    message: "Taking time for gratitude changes everything!",
  },
  {
    title: "Amazing! 🌈",
    message: "You're building a treasure of beautiful memories!",
  },
  {
    title: "Brilliant! 💖",
    message: "Every note adds to your happiness journey!",
  },
  {
    title: "Awesome! ⭐",
  ...
  {
    title: "Excellent! 🌻",
    message: "Small moments of gratitude create big joy!",
  },
  {
    title: "Magnificent! 🦋",
    message: "Your gratitude practice is blossoming!",
  },
];
...
const GRATITUDE_SUGGESTIONS = [
  ...
  {
    text: "Someone who believed in me when I doubted myself",
    type: "kindness",
  },
  ...
  {
    text: "A person who showed genuine interest in my ideas",
    type: "kindness",
  },
...
function updateJarVisuals() {
  ...
  jarCountEl.textContent = `${emoji} ${count} note${
    count !== 1 ? "s" : ""
  } in your jar ${emoji}`;
...
function exportNotes() {
  ...
  if (notes.length === 0) {
    ...
    modalContent.innerHTML =
      '<p class="text-center py-4">No entries yet. Start adding some gratitude notes to fill your jar!</p>';
    modal.classList.add("modal-open");
...
`;
  ...
  modalTitle.innerHTML =
    '📔 Your Gratitude Journal <span style="font-size: 0.9em">✨</span>';
  modalContent.innerHTML = content;
  modal.classList.add("modal-open");
}
...
function clearJar() {
  ...
  if (
    confirm(
      "Are you sure you want to clear your jar? This action cannot be undone."
    )
  ) {
...
function showToast(message) {
  ...
  toast.className =
    "fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-purple-800 text-white py-3 px-5 rounded-xl shadow-lg z-50";
...
function getRandomSuggestions(count = 5, focusType = "all") {
  ...
  const filteredSuggestions =
    focusType === "all"
      ...
      : GRATITUDE_SUGGESTIONS.filter(
          (s) => s.type === focusType || s.type === "kindness"
        );
  ...
  return shuffled.slice(0, count).map((suggestion) => {
    ...
    const emoji = getRandomItem(
      GRATITUDE_EMOJIS[type] || GRATITUDE_EMOJIS.help
    );
    return {
      ...suggestion,
      icon: emoji,
...
function selectSuggestion(index) {
  ...
  const previousSelectedEl = document.querySelector(
...
function openFeedbackModal() {
  ...
  // Track opening the feedback form
  if (window.va) {
    window.va("event", {
...
function setupFormspree() {
  feedbackForm.addEventListener('submit', function(event) {
    // Prevent the default form submission (which would cause page navigation)
    ...
    // Submit form data using fetch API instead of default form submission
    fetch(formAction, {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      ...
      if (window.va) {
        window.va("event", {
          ...
          data: {
            has_name: document.getElementById("feedback-name").value.trim().length > 0
          }
        });
      }
    })
...
function setupEventListeners() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape" && feedbackModal.classList.contains("modal-open")) {
      closeFeedbackModal();
    }
  });
  ...
  feedbackRetryBtn.addEventListener("click", function() {
    feedbackFormContainer.style.display = "block";
    feedbackError.classList.add("hidden");
  });
  feedbackErrorCloseBtn.addEventListener("click", closeFeedbackModal);
